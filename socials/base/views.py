...
class ShowProfilePageView(DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        # return {"hi": page_user.username}
        res = requests.post(
            "https://bluelink.pythonanywhere.com/is_verified",
            json={"username": page_user.username},
        ).text
        context["verified"] = res
        return context
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.add_message(
                request, messages.INFO, "You have successfully logged in."
            )
            return redirect("/")

        else:
            messages.add_message(
                request, messages.INFO, "Invalid username or password."
            )
...
def signup(request):
    ...
    if request.method == "POST":
        ...
        if email == "" or password == "" or username == "":
            ...
            auth_login(request, user)
            messages.add_message(
                request, messages.INFO, "You have successfully signed up."
            )
